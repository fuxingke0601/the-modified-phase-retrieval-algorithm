...
!      nu = 2*cell(1)*sqrt(maxres); nv = 2*cell(2)*sqrt(maxres); nw = 2*cell(3)*sqrt(maxres)
      ...
      write(6,'(/,a,F8.2,F8.2/)') '弱点设置的阈值和实际阈值 : ',intensity(nint(weak*nrefls)+1), cutoff
      print*, '用于计算sayre等式的衍射点的数目和阈值 : ', sayre_percent,sayre_cutoff
      write(6,'(/,a,I4/)') '弱衍射点的数目 : ', j
      print*,"设置的弱点的数目，missing点的数目，设置的阈值百分比",nint(weak*nrefls),nrefls-num_reobs,weak
      write(6,'(/,a,/)') '*************************************************************'
...
!       end if
	 ...
	 sf = dcmplx(0,0)
      ...
      !end if  
         !再加一步，重新计算sf，把修改后的相位导入sf中,这里把没有修正的衍射点的强度都设为0
         sf = dcmplx(0,0)
         do i=1,refl_refine_num
            ii=refl_refine(i,1)              !表示现在处理第ii个衍射点的相位
            do j = 1, nsymp
