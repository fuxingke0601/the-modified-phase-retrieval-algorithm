...
module functions
   ...
   real sayre_percent     !计算sayre等式时，等式右边所包含的衍射的百分比
...
!      nu = 2*cell(1)*sqrt(maxres); nv = 2*cell(2)*sqrt(maxres); nw = 2*cell(3)*sqrt(maxres)
      ...
      sayre_cutoff=intensity(nint((1-sayre_percent)*num_reobs)+1)
      ...
      print*, '用于计算sayre等式的衍射点的数目和阈值 : ',nint(sayre_percent*num_reobs), sayre_cutoff
      write(6,'(/,a,I4/)') '修正的衍射点的数目 : ', j
      write(6,'(/,a,/)') '*************************************************************'
   ...
   logical:: is_unique1, is_unique2
      ...
      do i=1,nrefls-1
         if(.not. hkls(i)%missing)then
         if (hkls(i)%amp .gt. sayre_cutoff)then !只使用部分衍射点计算sayre等式
            t=t+1
            hkl_test=0
            do j = 1, nsymp
               h = hkls(i)%h*rsym(1,1,j)+hkls(i)%k*rsym(2,1,j)+hkls(i)%l*rsym(3,1,j)
   ...
   subroutine triplets_search(refl_refine,triplets_k,triplets_hk,refl_refine_num,num_refl,hkl_array,triplet_num)
      implicit none
      ...
      integer i,j,refl_refine_num,k_num                                 !refl_refine_num表示需要修正的衍射点的个数
      integer sf(0:nu-1,0:nv-1,0:nw-1)
      real phase

      real(8) ,allocatable :: amp_k_hk(:,:), amp_k_hk_tmp(:,:)             !存储E(k)*E(h-k)的计算结果
      complex(8) ::  sf_amp(0:nu-1,0:nv-1,0:nw-1)            !为了存储hkls(i)%amp，方便后边计算E(k)*E(h-k)
      integer triplet_num                                               !设定用于相位修正的tripler的数目
      real(8),allocatable :: thre_triplet(:),thre_triplet_tmp(:)           !thre_triplet用于存储计算triplet_num的阈值
      real(8) tmp_1,tmp_2
      integer tmp1,tmp2

      integer:: h, k, l, hk_h, hk_k, hk_l, h_max, k_max, l_max, tt_tmp,min_triplet
      ...
      do i = 1, nrefls-1
      if (.not. hkls(i)%missing) then
      if (hkls(i)%amp .gt. sayre_cutoff)then !只使用部分衍射计算sayre等式
         do j = 1, nsymp
            h = hkls(i)%h*rsym(1,1,j)+hkls(i)%k*rsym(2,1,j)+hkls(i)%l*rsym(3,1,j)
            k = hkls(i)%h*rsym(1,2,j)+hkls(i)%k*rsym(2,2,j)+hkls(i)%l*rsym(3,2,j)
            l = hkls(i)%h*rsym(1,3,j)+hkls(i)%k*rsym(2,3,j)+hkls(i)%l*rsym(3,3,j)
            ...
            sf(lmod(-h,nu),lmod(-k,nv),lmod(-l,nw)) = 1
            sf_amp(lmod(h,nu),lmod(k,nv),lmod(l,nw)) = dcmplx(hkls(i)%amp*cos(phase),hkls(i)%amp*sin(phase))
            sf_amp(lmod(-h,nu),lmod(-k,nv),lmod(-l,nw)) = dcmplx(hkls(i)%amp*cos(phase),-hkls(i)%amp*sin(phase))
         end do
      end if
      end if
      ...
      !计算triplets_k，只存储E(k)*E(H-K)较大的前1000个triplets
      allocate(triplets_k_tmp(refl_refine_num,3,num_refl))
      allocate(triplets_hk_tmp(refl_refine_num,3,num_refl))
      allocate(amp_k_hk_tmp(refl_refine_num,num_refl))

      h_max=maxval(hkl_array(1,1:num_refl)); k_max=maxval(hkl_array(2,1:num_refl)); l_max=maxval(hkl_array(3,1:num_refl));
      min_triplet=99999                   !计算所有衍射点中，triplets的数目的最小值
      ...
      do i=1,refl_refine_num
         ...
         do j=1,num_refl
            ...
            .and.  sf(lmod(hk_h,nu),lmod(hk_k,nv),lmod(hk_l,nw)) .gt. 0) then !防止hk_h,hk_k,hk_l超出sf的范围
               ...
               !计算E(K)*E(H-K)
               tmp_1=cdabs(sf_amp(lmod(hkl_array(1,j),nu),lmod(hkl_array(2,j),nv),lmod(hkl_array(3,j),nw)))
               tmp_2=cdabs(sf_amp(lmod(hk_h,nu),lmod(hk_k,nv),lmod(hk_l,nw)))
               amp_k_hk_tmp(i,k_num)=tmp_1*tmp_2
            end if
         end do
         
         !call quicksort(amp_k_hk_tmp(1:k_tmp),1,k_tmp,1,1,k_tmp)
         ...
         if (k_num .lt. min_triplet)then
            min_triplet=k_num
         end if
      end do
      
      !对于存储E(K)*E(H-K)的数据进行内存分配
      if (triplet_num .gt. min_triplet)then
         triplet_num=min_triplet
      end if
      allocate(amp_k_hk(refl_refine_num,tt_tmp))
      amp_k_hk=amp_k_hk_tmp(1:refl_refine_num,1:tt_tmp)
      deallocate(amp_k_hk_tmp)

      !计算确定哪些triplets用于计算所需的阈值，thre_triplet
      allocate(thre_triplet(refl_refine_num))
      do i=1,refl_refine_num
         tmp1=refl_refine(i,2)
         allocate(thre_triplet_tmp(tmp1))
         thre_triplet_tmp=amp_k_hk(i,1:tmp1)
         call quicksort(thre_triplet_tmp,1,tmp1,1,1,tmp1)
         tmp2=tmp1-triplet_num
         if(tmp2 .gt. 0)then
            thre_triplet(i)=thre_triplet_tmp(tmp2)
         else
            thre_triplet(i)=0.0
         end if
         deallocate(thre_triplet_tmp)
      end do

      !allocate(triplets_k(refl_refine_num,3,triplet_num))
      ...
      !计算triplets_hk
      allocate(triplets_k(refl_refine_num,3,triplet_num))
      allocate(triplets_hk(refl_refine_num,3,triplet_num))
      triplets_k=0
      triplets_hk=0
      do i=1,refl_refine_num
         tmp1=0
         do j=1,refl_refine(i,2)
            if (amp_k_hk(i,j) .gt. thre_triplet(i) )then
               tmp1=tmp1+1
               triplets_k(i,1,tmp1)=triplets_k_tmp(i,1,j)
               triplets_k(i,2,tmp1)=triplets_k_tmp(i,2,j)
               triplets_k(i,3,tmp1)=triplets_k_tmp(i,3,j)
               triplets_hk(i,1,tmp1)=triplets_hk_tmp(i,1,j)
               triplets_hk(i,2,tmp1)=triplets_hk_tmp(i,2,j)
               triplets_hk(i,3,tmp1)=triplets_hk_tmp(i,3,j)
            end if
         end do
      end do
   ...
   subroutine charge_flipping
      implicit none
      integer n, tt, n_multi_real, s, x, y, z, xx, yy, zz, i, j, h, k, l, m, reci, nbest, p,q,r,ii !reci表示倒空间中使用哪种约束，1表示使用Fo替代Fc，2表示使用2Fo-Fc替代Fc
      ...
      !找到所有的三相位关系
      triplet_num=1000
      call triplets_search(refl_refine,triplets_k,triplets_hk,refl_refine_num,num_refl,hkl_array,triplet_num)
...
!       end if
	 ...
	 sf = dcmplx(0,0)
      ...
      if (sayre .eq. 1)then
          ...
          sf_sayre=sf
          !$OMP PARALLEL DO PRIVATE(i,j,sum_tmp,Fhk_tmp,Fk_tmp) SHARED(sf_sayre,hkls)
          do i=1,refl_refine_num
             sum_tmp=dcmplx(0,0)
          ...
          !再加一步，重新计算sf，把修改后的相位导入sf中,这里把missing点的强度都设为0
         do i = 1, nrefls-1
         if (.not. hkls(i)%weak) then
            do j = 1, nsymp
               h = hkls(i)%h*rsym(1,1,j)+hkls(i)%k*rsym(2,1,j)+hkls(i)%l*rsym(3,1,j)
   ...
   subroutine pearson(pearson_correl)
        ...
        do i=1,nrefls
           if ((.not. hkls(i)%missing) .and. (hkls(i)%sigf .gt. 0.0001) .and. (hkls(i)%amp .lt. 2.1) &
                ...
                if ( hkls(i)%sigf .lt. 0.608 ) then  !0.4072,5c82
                   ngood=ngood+1
                   x=x+hkls(i)%amp
                   y=y+hkls(i)%gnorm
