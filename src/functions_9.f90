...
!function_8.f90, 在functions.f90基础上，加入Acta Cryst. (2007). A63,400–406文献中正切公式的权重因子,并在这个版本的基础上测试W-R和W-90
!function_9.f90，在functions_8.f90的基础上，测试W-R和W-90，设置弱衍射点时同时分为包含missing点和不包含missing点
module functions
   use omp_lib
...
50    continue
   ...
   subroutine mtzin
      ...
      integer num_reobs,j           !mtz中观测到的衍射点的个数
...
!      nu = 2*cell(1)*sqrt(maxres); nv = 2*cell(2)*sqrt(maxres); nw = 2*cell(3)*sqrt(maxres)
      ...
      cutoff = intensity(nint(weak*num_reobs)+1)                 !计算弱衍射所需要的阈值
      sayre_cutoff=intensity(num_reobs-sayre_percent+1)          !计算sayre等式时，选取最强衍射点时所用的阈值
      ...
      print*,nint(weak*num_reobs),weak,num_reobs
      write(6,'(/,a,/)') '*************************************************************'
