...
!function_4.f90, 加入sayre等式，使用没有丢失的比较强的E0计算sayre等式，修正部分not missing中强衍射点的相位，这里正空间中，先计算abs,cutoff,raar,abs
!function_5.f90，在function_4.f90的基础上，优化算法，提高计算速率。方法为先设定义一个函数，找全部的三个组合，然后保存到一个数组里面，后面直接调用这个数组
!之前测试的function_5.f90有问题，triplets_hk计算不对，现在是改正过的，但还没有测试
!function_6.f90，在function_5.f90的基础上, 找出一定数目的triplets，用于优化相位。方法是根据|EK|*|EH-K|进行排序
!function_7.f90, 在2023.08.22跟郝权老师讨论完之后，使用的衍射点数目是最强的500个或者1000个衍射点,或者更多衍射点
!function_8.f90, 在functions_7.f90基础上，加入Acta Cryst. (2007). A63,400–406文献中正切公式的权重因子,并在这个版本的基础上测试W-R和W-90
!function_9.f90，在functions_8.f90的基础上，测试W-R和W-90，设置弱衍射点时同时分为包含missing点和不包含missing点,这里只测试包含W-R和W-90不包含missing点时的情况
!function_10.f90，在functions_8.f920的基础上，测试W-R和W-90，设置弱衍射点包含missing点时的情况
!function_11.f90，程序function_10，没有确定W90的参数和直接法参数(强衍射点数目),这里是测试确定这些参数
module functions
   use omp_lib
   ...
   real sayre_cutoff  !定义使用sayre等式时，等式右边所包含的衍射的衍射强度E的最小值，即Ek的最小值
   ...
   integer sayre_input          !设定是否使用sayre方程修正相位，1表示使用，0表示不使用
   integer sayre_percent  !计算sayre等式时，所使用的最强衍射点的数目
   real    pearson_thro   !计算pearson相关系数时，所使用的阈值
...
50    continue
   ...
   subroutine mtzin
      ...
      real(8),allocatable :: intensity(:),intensity2(:)
      ...
      integer num_reobs,j,tmp1           !mtz中观测到的衍射点的个数
...
10    continue
      ...
      else if(key .eq. 'CCTR')then
         pearson_thro = fvalue(2)
         goto 10
      else if(key .eq. 'SCUT')then
         sayre_percent = fvalue(2)
         goto 10
      else if(key .eq. 'SAY')then
         sayre_input = fvalue(2)
         goto 10
...
!      nu = 2*cell(1)*sqrt(maxres); nv = 2*cell(2)*sqrt(maxres); nw = 2*cell(3)*sqrt(maxres)
      ...
      do i = 1, nrefls
         if (.not. hkls(i)%missing)then
            ...
            num_reobs=num_reobs+1
         end if
      end do
      print*,"not missing reflections :         ",num_reobs

      !计算用于sayre等式最强衍射点所需的阈值，
      allocate(intensity(num_reobs))
      j=0
      do i = 1, nrefls
         if (.not. hkls(i)%missing)then
            j=j+1
            intensity(j) = hkls(i)%amp
            !num_reobs=num_reobs+1
         end if
      end do
      call quicksort(intensity,1,num_reobs,1,1,num_reobs)
      cutoff = intensity(nint(weak*num_reobs)+1)
      sayre_cutoff=intensity(num_reobs-sayre_percent+1)
      ...
      !如果要用直接发，用重新计算sayre_cufoff
      !sayre_cutoff=1.2
      
      !计算弱衍射点，弱衍射点包括missing点
      !allocate(intensity(nrefls))
      !do i = 1, nrefls
      !   intensity(i) = hkls(i)%amp
      !end do
      !call quicksort(intensity,1,nrefls,1,1,nrefls)

      !tmp1=nrefls-num_reobs !missing点的数目
      !if(tmp1 .lt. nint(weak*nrefls))then
      !   tmp1=nint(weak*nrefls)
      !end if
      !cutoff = intensity(tmp1+1)
      ...
      !write(6,'(/,a,F8.2,F8.2,F8.2/)') '弱点设置的阈值和实际阈值,设置的阈值百分:',intensity(nint(weak*nrefls)+1),cutoff,weak
      print*,'弱点设置的阈值和实际阈值,设置的阈值百分:',intensity(nint(weak*num_reobs)+1),cutoff,weak
      print*, '用于计算sayre等式的衍射点的数目和阈值 : ', sayre_percent,sayre_cutoff
      print*, '弱衍射点的数目 : ', j
      print*,"观测点中设置的弱点的数目，missing点的数目",nint(weak*num_reobs),nrefls-num_reobs
      write(6,'(/,a,/)') '*************************************************************'
   ...
   logical:: is_unique1, is_unique2
      ...
      do i=1,nrefls-1
         if(.not. hkls(i)%missing)then
         if (hkls(i)%amp .ge. sayre_cutoff)then !只使用部分衍射点计算sayre等式
            t=t+1
            hkl_test=0
            do j = 1, nsymp
               h = hkls(i)%h*rsym(1,1,j)+hkls(i)%k*rsym(2,1,j)+hkls(i)%l*rsym(3,1,j)
      ...
      !num_refl=num_refl+1
      !hkl_array(1,num_refl)=0; hkl_array(2,num_refl)=0; hkl_array(3,num_refl)=0
      print*,"用于修正的总的衍射点数目，观测到的衍射点数目，设置的衍射点数目",num_refl,nrefls,t

   return
   end subroutine refl_unit

   subroutine triplets_search(refl_refine,triplets_k,triplets_hk,refl_refine_num,num_refl,hkl_array)
      implicit none
      integer num_refl, hkl_array(3,2*(nrefls-1)*nsymp+1)
      integer,allocatable :: triplets_k(:,:,:), triplets_k_tmp(:,:,:)   !第一个维度表示需要修正的衍射点个数，第二个维度表示坐标x,y,z，第三个维度表示triplets的k对应的坐标
      integer,allocatable :: refl_refine(:,:),refl_refine_tmp(:,:)      !第一个维度表示需要修正的是第几个衍射点的相位，第二个维度表示这个衍射点对应的triplets的数目     
      integer,allocatable :: triplets_hk(:,:,:), triplets_hk_tmp(:,:,:) !第一个维度表示需要修正的衍射点个数，第二个维度表示坐标x,y,z，第三个维度表示triplets的h-k对应的坐标
      integer i,j,refl_refine_num,k_num,tmp1                            !refl_refine_num表示需要修正的衍射点的个数
      integer sf(0:nu-1,0:nv-1,0:nw-1)

      integer:: h, k, l, hk_h, hk_k, hk_l, h_max, k_max, l_max, tt_tmp

      !得到一个三维矩阵，用于计算sayre等式的衍射点处赋值为1，否则为0，方便计算triplets
      sf = 0
      do i = 1, nrefls-1
      if (.not. hkls(i)%missing) then
      if (hkls(i)%amp .ge. sayre_cutoff)then !只使用部分衍射计算sayre等式
         do j = 1, nsymp
            h = hkls(i)%h*rsym(1,1,j)+hkls(i)%k*rsym(2,1,j)+hkls(i)%l*rsym(3,1,j)
            k = hkls(i)%h*rsym(1,2,j)+hkls(i)%k*rsym(2,2,j)+hkls(i)%l*rsym(3,2,j)
            l = hkls(i)%h*rsym(1,3,j)+hkls(i)%k*rsym(2,3,j)+hkls(i)%l*rsym(3,3,j)
            sf(lmod(h,nu),lmod(k,nv),lmod(l,nw)) = 1
            sf(lmod(-h,nu),lmod(-k,nv),lmod(-l,nw)) = 1
         end do
      end if
      end if
      ...
      !首先计算需要修正的衍射点的下标即hkls(i)中i, 和对应的triplets的数目
      allocate(refl_refine_tmp(nrefls,2))
      refl_refine_tmp=0
      refl_refine_num=0
      do i=1,nrefls-1
         if(.not. hkls(i)%missing)then
         if (hkls(i)%amp .ge. sayre_cutoff)then
            refl_refine_num=refl_refine_num+1
            refl_refine_tmp(refl_refine_num,1)=i
         end if
         end if
      end do
      print*,"要修正的衍射点数目：",refl_refine_num

      allocate(refl_refine(refl_refine_num,2))
      refl_refine=refl_refine_tmp(1:refl_refine_num,1:2)
      deallocate(refl_refine_tmp)
      !print*,size(refl_refine,dim=1),size(refl_refine,dim=2),refl_refine_num
      
      allocate(triplets_k_tmp(refl_refine_num,3,num_refl))
      allocate(triplets_hk_tmp(refl_refine_num,3,num_refl))

      h_max=maxval(hkl_array(1,1:num_refl)); k_max=maxval(hkl_array(2,1:num_refl)); l_max=maxval(hkl_array(3,1:num_refl));
      tt_tmp=0                            !计算所有衍射点中，triplets的数目的最大值
      tmp1=0
      do i=1,refl_refine_num
         k_num=0
         do j=1,num_refl
            hk_h=hkls(refl_refine(i,1))%h-hkl_array(1,j)
            hk_k=hkls(refl_refine(i,1))%k-hkl_array(2,j)
            hk_l=hkls(refl_refine(i,1))%l-hkl_array(3,j)
            if(abs(hk_h).le.h_max .and. abs(hk_k).le.k_max .and. abs(hk_l).le.l_max &
            .and.  sf(lmod(hk_h,nu),lmod(hk_k,nv),lmod(hk_l,nw)) .gt. 0) then !防止hk_h,hk_k,hk_l超出sf的范围
               k_num=k_num+1
               triplets_k_tmp(i,1,k_num)=lmod(hkl_array(1,j),nu)
               triplets_k_tmp(i,2,k_num)=lmod(hkl_array(2,j),nv)
               triplets_k_tmp(i,3,k_num)=lmod(hkl_array(3,j),nw)

               triplets_hk_tmp(i,1,k_num)=lmod(hk_h,nu)
               triplets_hk_tmp(i,2,k_num)=lmod(hk_k,nv)
               triplets_hk_tmp(i,3,k_num)=lmod(hk_l,nw)
            end if
         end do
         tmp1=tmp1+k_num
         refl_refine(i,2)=k_num           !存储每个衍射点对应的triplets的数目
         if (k_num .gt. tt_tmp)then
            tt_tmp=k_num
         end if
      end do
      
      allocate(triplets_k(refl_refine_num,3,tt_tmp))
      triplets_k=triplets_k_tmp(1:refl_refine_num,1:3,1:tt_tmp)
      deallocate(triplets_k_tmp)
      allocate(triplets_hk(refl_refine_num,3,tt_tmp))
      triplets_hk=triplets_hk_tmp(1:refl_refine_num,1:3,1:tt_tmp)
      deallocate(triplets_hk_tmp)

      print*, "三相位关系数目的最小值和最大值，平均值:",minval(refl_refine(1:refl_refine_num,2)),&
             maxval(refl_refine(1:refl_refine_num,2)), tmp1/refl_refine_num
      return
   end subroutine triplets_search


   !subroutine   thre_triplet_cal(amp_k_hk(i,1:refl_refine(i,2)),refl_refine(i,2),tmp_1)
   !   implicit none
   !end subroutine   thre_triplet_cal
   ...
   if (sayre .eq. 0)then
      ...
      do i=1,nrefls-1
         if(.not. hkls(i)%weak)then
            ...
            hkls(i)%opha = hkls(i)%opha + 1.0*(atan2(aimag(sum_tmp),real(sum_tmp))-hkls(i)%opha)           !相位使用sayre公式修正后的相位 
         end if
      end do
   end if
   !print*,atan2(aimag(sum_tmp),real(sum_tmp)), hkls(i-2)%opha
   ...
   subroutine charge_flipping
      implicit none
      integer tt, n_multi_real, s, x, y, z, xx, yy, zz, i, j, h, k, l, m, reci, nbest, p,q,r,ii !reci表示倒空间中使用哪种约束，1表示使用Fo替代Fc，2表示使用2Fo-Fc替代Fc
      integer(8)  n
      ...
      integer,allocatable :: triplets_k(:,:,:)     !第一个维度表示需要修正的衍射点个数，第二个维度表示坐标x,y,z，第三个维度表示triplets的k对应的坐标
      integer,allocatable :: refl_refine(:,:)      !第一个维度表示需要修正的是第几个衍射点的相位，第二个维度表示这个衍射点对应的triplets的数目
      integer,allocatable :: triplets_hk(:,:,:)    !第一个维度表示需要修正的衍射点个数，第二个维度表示坐标x,y,z，第三个维度表示triplets的h-k对应的坐标
      integer refl_refine_num                      !refl_refine_num表示需要修正的衍射点的个数
      complex(8) sum_tmp,Fk_tmp,Fhk_tmp
      integer triplet_num                          !设定用于相位修正的tripler的数目
      real(8),allocatable :: Ephi_sayre(:,:)       !使用sayre公式修正后的振幅和相位存储到一个数组中，第一个为修正后的振幅*E(h)，第二个为修正后的相位
      integer sayre
      ...
      !找到所有的三相位关系
      triplet_num=1000
      call triplets_search(refl_refine,triplets_k,triplets_hk,refl_refine_num,num_refl,hkl_array)
      
      !分配内存给Ephi_sayre，用于存储sayre等式修正后的振幅和相位
      allocate(Ephi_sayre(refl_refine_num,2))
      ...
      outer: do m = 1, ntry
       ...
       !if ((m .lt. 100) .or. (m .gt. 300))then
       !if ((m .ne. 234) .and. (m .ne. 193) .and.(m .ne. 169) .and. (m .ne. 168) .and.(m .ne. 151).and. (m .ne. 115) &
!.and. (m .ne. 261) .and.(m .ne. 308).and. (m .ne. 358) .and.(m .ne. 363).and. (m .ne. 399).and. (m .ne. 72) &
!.and. (m .ne. 93) .and. (m .ne. 110))then
!          cycle
!       end if
       !计算初始相位得到的map图
       ...
       hkls_best=hkls
       n=1
       call ApplyReciRestr(sf,reci,reci_thr_diff,n) !sf(0,0,0)设为0
	 ...
	 !set outer resolution and systematic absent reflections always to be 0, else comment next line to retain last cycled sf
         ...
         reci_thr_diff=0 !表示the absolute value anologue of w parameter of the Fobs+DeltaF idea (Oszlanyi&Suto, 2008)
	 sf = dcmplx(0,0)
         call ApplyReciRestr(sf,reci,reci_thr_diff,n) !sf(0,0,0)设为0,这里的n是伪随机数种子
         
         !应用sayre等式修正相位
      if(sayre_input .eq. 1)then
         ...
         !sayre=0
      if (sayre .eq. 1)then
          !allocate(Ephi_sayre(refl_refine_num,2))        !由于每次都要重新分配内存，所以在大循环外分配内存
          sf_sayre=sf
          !$OMP PARALLEL DO PRIVATE(i,j,sum_tmp,Fhk_tmp,Fk_tmp) SHARED(Ephi_sayre,hkls)
          do i=1,refl_refine_num
             sum_tmp=dcmplx(0,0)
             !ii=refl_refine(i,1)              !表示现在处理第ii个衍射点的相位
             do j=1,refl_refine(i,2)
                  Fhk_tmp=sf_sayre(triplets_hk(i,1,j),triplets_hk(i,2,j),triplets_hk(i,3,j))
                  Fk_tmp=sf_sayre(triplets_k(i,1,j),triplets_k(i,2,j),triplets_k(i,3,j))
                  sum_tmp=sum_tmp+Fhk_tmp * Fk_tmp
             end do

             Ephi_sayre(i,1)=cdabs(sum_tmp)*hkls(refl_refine(i,1))%amp
             Ephi_sayre(i,2)=atan2(aimag(sum_tmp),real(sum_tmp))
             !hkls(ii)%opha = hkls(ii)%opha + 1.0*(atan2(aimag(sum_tmp),real(sum_tmp))-hkls(ii)%opha)           !相位使用sayre公式修正后的相位
             !hkls(refl_refine(i,1))%opha = hkls(refl_refine(i,1))%opha + &
             !        1.0*(atan2(aimag(sum_tmp),real(sum_tmp))-hkls(refl_refine(i,1))%opha)
             
             !修正sf中的相位
             !ii=refl_refine(i,1)
             !do p = 1, nsymp
             !   h = hkls(ii)%h*rsym(1,1,p)+hkls(ii)%k*rsym(2,1,p)+hkls(ii)%l*rsym(3,1,p)
             !   k = hkls(ii)%h*rsym(1,2,p)+hkls(ii)%k*rsym(2,2,p)+hkls(ii)%l*rsym(3,2,p)
             !   l = hkls(ii)%h*rsym(1,3,p)+hkls(ii)%k*rsym(2,3,p)+hkls(ii)%l*rsym(3,3,p)
             !   phase= hkls(ii)%opha-2*pi*(hkls(ii)%h*rsym(1,4,p)+hkls(ii)%k*rsym(2,4,p)+hkls(ii)%l*rsym(3,4,p))
                !tmp1=0; tmp2=0
             !   tmp1=hkls(ii)%amp*cos(phase)
             !   tmp2=hkls(ii)%amp*sin(phase)
             !   sf(lmod(h,nu),lmod(k,nv),lmod(l,nw)) = dcmplx(tmp1, tmp2)
             !   sf(lmod(-h,nu),lmod(-k,nv),lmod(-l,nw)) = dcmplx(tmp1,-tmp2)
             !end do
          ...
          !$OMP END PARALLEL DO
          !计算sayre等式优化后带权重的相位，并存入hkls中
          tmp1=maxval(Ephi_sayre(1:refl_refine_num,1))           !计算权重因子的归一化系数;
          do i=1,refl_refine_num
             tmp1=maxval(Ephi_sayre(1:refl_refine_num,1))          
             hkls(refl_refine(i,1))%opha = hkls(refl_refine(i,1))%opha + &
                     Ephi_sayre(i,1)/tmp1*(Ephi_sayre(i,2)-hkls(refl_refine(i,1))%opha)
          end do
      ...
      !end if  
         ...
         do i=1,refl_refine_num
            ii=refl_refine(i,1)              !表示现在处理第ii个衍射点的相位
            do j = 1, nsymp
               h = hkls(ii)%h*rsym(1,1,j)+hkls(ii)%k*rsym(2,1,j)+hkls(ii)%l*rsym(3,1,j)
               k = hkls(ii)%h*rsym(1,2,j)+hkls(ii)%k*rsym(2,2,j)+hkls(ii)%l*rsym(3,2,j)
               l = hkls(ii)%h*rsym(1,3,j)+hkls(ii)%k*rsym(2,3,j)+hkls(ii)%l*rsym(3,3,j)
               phase= hkls(ii)%opha-2*pi*(hkls(ii)%h*rsym(1,4,j)+hkls(ii)%k*rsym(2,4,j)+hkls(ii)%l*rsym(3,4,j))
               tmp1=0; tmp2=0
               tmp1=hkls(ii)%amp*cos(phase)
               tmp2=hkls(ii)%amp*sin(phase)
               sf(lmod(h,nu),lmod(k,nv),lmod(l,nw)) = dcmplx(tmp1, tmp2)
         ...
         !print*, size(triplets_k,dim=1),size(triplets_k,dim=2),size(triplets_k,dim=3),minval(refl_refine(1:refl_refine_num,2)),&
         !maxval(refl_refine(1:refl_refine_num,2))
   ...
   subroutine pearson(pearson_correl)
        ...
        do i=1,nrefls
           if ((.not. hkls(i)%missing) .and. (hkls(i)%sigf .gt. 0.0001) .and. (hkls(i)%amp .lt. 2.1) &
                ...
                if ( hkls(i)%sigf .lt. pearson_thro ) then  !0.4072,5c82
                   ngood=ngood+1
                   x=x+hkls(i)%amp
                   y=y+hkls(i)%gnorm
    ...
    !倒空间限制
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    subroutine ApplyReciRestr(sf,reci,reci_thr_diff,n_fgsl)
        implicit none
        integer::i,j,h,k,l,reci !reci表示倒空间中使用哪种约束，1表示使用Fo替代Fc，2表示使用2Fo-Fc替代Fc
        complex(8)::sf(0:nu-1,0:nv-1,0:nw-1)
        real::newf,phase,tmp1,tmp2,reci_thr_diff,cphase !表示the absolute value anologue of w parameter of the Fobs+DeltaF idea (Oszlanyi&Suto, 2008)
        ...
        if (hkls(i)%weak) then
           ...
           !hkls(i)%opha=cphase*2*pi
           !cphase = fgsl_rng_uniform (r_fgsl)
           !phase=cphase*2*pi-2*pi*(hkls(i)%h*rsym(1,4,j)+hkls(i)%k*rsym(2,4,j)+hkls(i)%l*rsym(3,4,j))
           !W-90测试
           !phase= hkls(i)%opha+pi/2-2*pi*(hkls(i)%h*rsym(1,4,j)+hkls(i)%k*rsym(2,4,j)+hkls(i)%l*rsym(3,4,j))
           hkls(i)%opha= hkls(i)%opha+pi/2
        end if
        end if
        do j = 1, nsymp
            h = hkls(i)%h*rsym(1,1,j)+hkls(i)%k*rsym(2,1,j)+hkls(i)%l*rsym(3,1,j)
