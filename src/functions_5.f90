...
!      nu = 2*cell(1)*sqrt(maxres); nv = 2*cell(2)*sqrt(maxres); nw = 2*cell(3)*sqrt(maxres)
   ...
   subroutine triplets_search(refl_refine,triplets_k,triplets_hk,refl_refine_num,num_refl,hkl_array)
      ...
      !      hk_h=hkls(refl_refine(i,1))%h-triplets_k(i,1,j)
      !      hk_k=hkls(refl_refine(i,1))%k-triplets_k(i,2,j)
      !      hk_l=hkls(refl_refine(i,1))%l-triplets_k(i,3,j)

      !      triplets_hk(i,1,j)=lmod(hk_h,nu)
      !      triplets_hk(i,2,j)=lmod(hk_k,nv)
      !      triplets_hk(i,3,j)=lmod(hk_l,nw)
      !   end do
      !end do
